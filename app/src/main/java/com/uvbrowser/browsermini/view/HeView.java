...
import com.uvbrowser.browsermini.activity.ActivityUv;
import com.uvbrowser.browsermini.utils.HomePageUv;
import com.uvbrowser.browsermini.utils.HystoryTaskUv;
...
public class HeView extends WebView{
	ThemeUtilsUv theme;
	...
	* Public constructors of BeHeView
	 */
	public HeView(Context c, Activity activity){
		super(c);
		WEB_ACTIVITY = (ActionBarActivity) activity;
	}
	public HeView(Context context, ActionBarActivity activity, ProgressBar pBar, boolean Private, final EditText txt)  {
		super(activity);
		theme = new ThemeUtilsUv(activity);
		isPrivate = Private;
		P_BAR = pBar;
	...
	public void initializeSettings(){
		...
		PreferenceUtilsUv utils = new PreferenceUtilsUv(WEB_ACTIVITY);
	...
	* This method sets the current instance of the BeHeView to go private or not
	...
	public void loadHomepage(){
		...
		if(utils.getHomePage().equals("default")) {
			HomePageUv page = new HomePageUv(this, WEB_ACTIVITY.getApplication());
			Void[] b = null;
			page.execute(b);
	...
	public void loadHistory(){
		HystoryTaskUv task = new HystoryTaskUv(WEB_ACTIVITY,this);
		Void[] va = null;
		task.execute(va);
	}
	class IJavascriptHandler {
		...
		public void processContent(String aContent,String title) {
			...
			Intent mReading = new Intent(WEB_ACTIVITY, ActivityUv.class);
    ...
    public void loadHistoty(){
		HystoryTaskUv task = new HystoryTaskUv(this.getContext(),this);
		Void[] d = null;
		task.execute(d);
	}
